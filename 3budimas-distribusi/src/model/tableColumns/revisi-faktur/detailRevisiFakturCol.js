  ...
  try {
    ...
    if (shipping?.listDetailFakturShipping?.detailFaktur) {
      ...
      shipping.listDetailFakturShipping.detailFaktur.forEach((product) => {
        // Perbaikan: Konversi semua delivered ke level UOM1
        const kartonToUom1 =
...
export const detailRevisiFakturCol = [
  ...
  columnHelper.accessor(
    (row) => {
      // Konversi semua delivered ke level UOM1
      const kartonToUom1 =
        (row.karton_delivered || 0) * (row.konversi_level3 || 1);
      const boxToUom1 = (row.box_delivered || 0) * (row.konversi_level2 || 1);
      const piecesUom1 = row.pieces_delivered || 0;
  ...
  // Perbaikan: Kolom untuk Disc P3 (v3p) - gunakan mode "delivered"
  columnHelper.accessor(
    (row) => {
      const voucherStatus = getVoucherStatus(row);
      const totalOrderSubtotal = getTotalOrderSubtotal();
  ...
  // Perbaikan: Total diskon - gunakan mode "delivered"
  columnHelper.accessor(
    (row) => {
      const voucherStatus = getVoucherStatus(row);
      const totalOrderSubtotal = getTotalOrderSubtotal();
  ...
  // Perbaikan: Total harga - gunakan delivered dan mode "delivered"
  columnHelper.accessor(
    (row) => {
      const voucherStatus = getVoucherStatus(row);
      const totalOrderSubtotal = getTotalOrderSubtotal();
  ...
  columnHelper.accessor(
    ...
    ],
    {
      ...
      cell: (info) => {
        ...
        // Hitung semua diskon dengan fungsi yang sudah ada menggunakan mode "delivered"
