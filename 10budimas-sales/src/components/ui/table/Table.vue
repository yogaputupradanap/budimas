...
const table = useVueTable({
  ...
  meta: {
    updateData: (rowIndex, columnId, value) => {
      // Simpan state pagination hanya jika ada data
      ...
      // Hanya kembalikan pagination jika ada data dan halaman valid
      nextTick(() => {
        if (currentPagination && table.getPageCount() > 0) {
          // Pastikan pageIndex yang akan diset valid
          ...
          const safePagination = {
            ...currentPagination,
            pageIndex: Math.max(0, safePageIndex),
          };
          table.setPagination(safePagination);
        }
      });
    },
...
watch(
  () => props.tableData,
  (newVal) => {
    // Simpan pagination state hanya jika ada data
    const hasData = newVal && newVal.length > 0;
    const currentPagination = hasData
      ? { ...table.getState().pagination }
      : null;

    data.value = newVal;

    // Hanya coba mempertahankan pagination jika ada data dan halaman valid
    nextTick(() => {
      if (hasData && currentPagination && table.getPageCount() > 0) {
        // Verifikasi bahwa pageIndex masih valid setelah data berubah
        ...
        // Hanya set pagination jika halaman masih valid
        if (safePageIndex >= 0) {
          table.setPagination({
            ...currentPagination,
...
<template>
  <div v-if="!loading" class="tw-w-full">
    ...
    <div class="tw-p-4 tw-flex tw-flex-col tw-gap-6" v-else>
      <Toolbar :table="table" v-model="globalFilter" :hideSearch="hideSearch" v-if="!hideToolbar" />
      <div
        :class="[
