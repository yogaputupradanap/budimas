...
export const formatNominalPromo = (data) => {
  if (!data || data.nominal_promo === undefined || data.nominal_promo === null) {
    return 'Rp 0';
  }
  const nominal = parseFloat(data.nominal_promo) || 0;
  if (data.kategori_promo === 1) {
    return `${formatNumber(nominal)}%`;
  } else {
    return `Rp ${formatNumber(nominal)}`;
  }
};