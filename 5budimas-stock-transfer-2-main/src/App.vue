...
const {getCabang, getPrincipal, getArmada, getPerusahaan} = others;
...
const handleFirstPaintPage = async () => {
  ...
  await Promise.all([getCabang(), getPrincipal(), getArmada(), getPerusahaan()]);
};
...
const watchTitle = () => {
  ...
  document.title = `${title} | Budimas Stock Transfer`;
};

onMounted(() => handleFirstPaintPage());
