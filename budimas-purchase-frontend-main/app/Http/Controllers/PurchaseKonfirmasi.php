...
class PurchaseKonfirmasi extends Controller {

    public function index() {
        return view('contents.transaksi-konfirmasi-list', [
            'breadcrumb' => ['Transaksi Purchase', 'Konfirmasi']
        ]);
    }
    ...
    public function store(Request $data) {
        ...
        (new HTTPRequest)->url('/api/extra/purchase-transaksi/proses/konfirmasi')
                         ->insert($data->all())->response;

        return redirect()->route('transaksi-konfirmasi.index')->with('status', 1);
    }
