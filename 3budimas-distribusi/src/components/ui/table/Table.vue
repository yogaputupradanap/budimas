...
const updateParentData = defineEmits(["change", "openRowModal", "removeRow", "paginationChange"]);

const localstorage = localStorage.getItem(props.keyLocalStorage || "");
...
const pagination = ref({
  pageIndex: localstorage ? JSON.parse(localstorage).pageIndex : 0,
...
const table = useVueTable({
  ...
  onPaginationChange: (updaterOrValue) => {
    ...
    updateParentData("paginationChange", next);
  },
  getRowId: (row) => row.id,
...
const getPaginationRow = () => table.getState().pagination;
// console.log("Selected Rows: ", getSelectedRow());
