...
<template>
  <div
    class="tw-w-full tw-flex lg:tw-flex-row tw-flex-col tw-gap-4 lg:tw-justify-between tw-justify-start lg:tw-items-center tw-items-start tw-mt-2 tw-pb-4">
    <div class="tw-flex lg:tw-gap-2 tw-gap-1 tw-select-none tw-text-xs tw-break-all lg:tw-flex-row tw-flex-col">
    ...
    <div class="tw-flex tw-gap-1 tw-h-8">
      <button @click="() => table.previousPage()" :disabled="!table.getCanPreviousPage()" class="table-page-button">
        <i class="mdi mdi-chevron-left tw-text-lg"></i>
      </button>
      <div class="tw-flex tw-gap-1">
        <button class="table-page-button" v-for="firstPg in firstPage" :key="firstPg"
          @click="table.setPageIndex(firstPg - 1)" :disabled="firstPg === table.getState().pagination.pageIndex + 1">
          {{ firstPg }}
        </button>
        <button v-if="totalPage.total.length > 5 && currentPage > 3" class="table-page-button">
          ...
        </button>
        <button v-for="page in middlePage" :key="page" class="table-page-button"
          :disabled="page === table.getState().pagination.pageIndex + 1" @click="table.setPageIndex(page - 1)">
          {{ page }}
        </button>
        <button v-if="totalPage.total.length > 5" class="table-page-button">
          ...
        </button>
        <button class="table-page-button" v-for="last in lastPage" :key="last" @click="table.setPageIndex(last - 1)"
          :disabled="last === table.getState().pagination.pageIndex + 1">
          {{ last }}
        </button>
      </div>
      <button @click="() => table.nextPage()" :disabled="!table.getCanNextPage()" class="table-page-button">
