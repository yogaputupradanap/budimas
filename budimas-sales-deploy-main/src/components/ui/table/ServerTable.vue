...
const updateParentData = defineEmits(["openRowModal", "removeRow"]);
...
const table = useVueTable({
  ...
  meta: {
    removeRow: (keyFilter, value) => {
      const updatedData = data.value.filter((val) => val[keyFilter] != value);
      updateParentData("removeRow", updatedData);
    },
  },
});
</script>
  ...
  <div class="tw-w-full">
    <div class="tw-p-4 tw-flex tw-flex-col tw-gap-6">
      ...
      <div
        ...
        <table
          v-if="data.length"
          :class="[!tableWidth ? 'tw-w-full' : tableWidth, 'tw-min-h-[180px]']">
