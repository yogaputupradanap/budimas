...
const filterOptions = (field) => {
  if (!filterText.value || filterText.value == "")
    return (optionsArray.value = props.options);
...
watchEffect(() => {
  ...
  if (matchingOption) {
    selectedOptionText.value = matchingOption[props.textField || "text"];
  }
...
<template>
  <div
    ...
    <span
      ...
      <template v-if="selectedOptionText || selectedOptionText === 0">
        {{ selectedOptionText }}
      </template>
      <template v-else>
