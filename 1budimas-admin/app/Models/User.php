...
class User extends Model implements Authenticatable
{
    ...
    {
        // Mendapatkan Token Awal dari Panggilan API.
        $data = ['username' => $credentials['username'], 'password' => $credentials['password']];
        $tokensResponse = $this->select('/api/extra/getUserToken', 'get', $data);

        // Periksa apakah panggilan API berhasil dan responsnya tidak kosong.
        if ($tokensResponse->status() != 200 || empty($tokensResponse->first())) {
            return null;  // Kembalikan null jika panggilan API gagal atau mengembalikan respons kosong.
        }

        $tokens = $tokensResponse->first()->tokens;

        // Jika tokens kosong, kembalikan null.
        if (empty($tokens)) {
            return null;  // Kembalikan null jika token tidak tersedia.
        }

        // Data Kredensial Diambil dari Pengiriman Halaman Login Pengguna.
        // Jika Ada, Ini Akan Menghasilkan Data Pengguna Dari Basis Data.
        ...
        if ($user->count() > 0) {
            $this->id = $user->first()->id;
            $this->username = $user->first()->username;
            $this->password = $user->first()->password;
            return $this;  // Kembalikan instance saat ini jika data user ditemukan.
        }

        return null;  // Kembalikan null jika tidak ada user yang ditemukan.
    }
    ...
    function getListByJabatanSales()
    {
        return $this
            ->token(Auth::user()->getRememberToken())
            ->select('/api/extra/getUserByIdJabatanSales')
            ->get();
    }
