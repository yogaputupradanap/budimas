...
export const listProdukColumns = (data) => [
            ...
            watch(uom3,
                (newValue) => {
                    ...
                    if (newValue >= 0) {
                        ...
                        data[info.row.index].total = Number(data[info.row.index].stock) * Number(info.row.original.harga_produk);
                        data[info.row.index].selisih = Number(data[info.row.index].stock) - Number(data[info.row.index].stock_system);
                    }
    ...
    columnHelper.display({
        ...
        header: "Selisih",
        cell: (info) =>
            <p class="tw-w-52 tw-text-base md:tw-w-auto">{data[info.row.index].selisih}</p>
    }),
    columnHelper.accessor((row) => row.stock_system, {
        id: "stock system",
        header: "Stock System",
        cell: (info) =>
            h("div", { class: "tw-w-52 tw-text-base md:tw-w-auto", innerText: info.getValue() }),
