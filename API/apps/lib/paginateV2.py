...
class PaginateV2():
    ...
    def __setColumns(self):
        ...
        if columns :
            ...
            self.filterColumns = [col.split('as')[0].strip() for col in array_columns if 'COUNT' not in col.upper() and '*' not in col]
    ...
    def paginate(self):
        ...
        {self.query} {self.string_group_by})
        """
        ...
        indexed_page_query = (
            f"""
                ...
                numbered_data AS (
                    ...
                    *
                    FROM summarized_data {clause_query or ''}
                )
                SELECT *
                FROM numbered_data
                {
