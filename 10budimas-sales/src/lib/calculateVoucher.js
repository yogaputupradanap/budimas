...
export const calculateTotalDiscount = (
  ...
  const result = {
    ...
    totalDiskon: 0,
    hargaSetelahDiskon: round2(totalHarga),
    hasInvalidVoucher: false
  };

  // Step 1: Voucher 1 Regular
  if (voucherSelections.voucher1Regular) {
    ...
    if (v1rResult.isValid) {
      result.discountDetails.diskon1Regular = round2(v1rResult.value);
    } else {
      result.hasInvalidVoucher = true;
    }
  }

  const hargaSetelahV1R = round2(
    totalHarga - result.discountDetails.diskon1Regular
  );

  // Step 2: Voucher 2 Regular
  if (voucherSelections.voucher2Regular) {
    ...
    if (v2rResult.isValid) {
      result.discountDetails.diskon2Regular = round2(v2rResult.value);
    } else {
      result.hasInvalidVoucher = true;
    }
  }

  const hargaSetelahV2R = round2(
    hargaSetelahV1R - result.discountDetails.diskon2Regular
  );

  // Step 3: Voucher 3 Regular
  if (voucherSelections.voucher3Regular) {
    ...
    if (v3rResult.isValid) {
      result.discountDetails.diskon3Regular = round2(v3rResult.value);
    } else {
      result.hasInvalidVoucher = true;
    }
  }

  const hargaSetelahV3R = round2(
    hargaSetelahV2R - result.discountDetails.diskon3Regular
  );

  // Step 4: Voucher 2 Product
  if (voucherSelections.voucher2Product) {
    ...
    if (v2pResult.isValid) {
      result.discountDetails.diskon2Product = round2(v2pResult.value);
    } else {
      result.hasInvalidVoucher = true;
    }
  }

  const hargaSetelahV2P = round2(
    hargaSetelahV3R - result.discountDetails.diskon2Product
  );

  // Step 5: Voucher 3 Product
  if (voucherSelections.voucher3Product) {
    ...
    if (v3pResult.isValid) {
      result.discountDetails.diskon3Product = round2(v3pResult.value);
    } else {
      result.hasInvalidVoucher = true;
    }
  }

  // Hitung total diskon & harga akhir
  result.totalDiskon = round2(
  ...
  result.hargaSetelahDiskon = round2(totalHarga - result.totalDiskon);
