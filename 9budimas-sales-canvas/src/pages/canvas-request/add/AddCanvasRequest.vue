...
import { listAddCanvasRequestColumn } from "@/src/model/tableColumns/canvas-request/tambah-canvas";
...
import { simpleDateNow, parseCurrency, breakdownToUOM, convertToTotalPieces } from "@/src/lib/utils";
...
const submitCanvasRequest = async () => {
  ...
  try {
    await salesCanvasService.createCanvasRequest(payload);
...
const tableMeta = {
  updateRow: (rowData, rowIndex, columnId, funcId) => {
...
watch(() => userStore.user.value, (user) => {
  if (user?.id) {
    fetchUrl.value = `${endpoints}/list-product-canvas?id=${user.id}`;
  }
}, { immediate: true });
    ...
    >
        <Card no-subheader>
            ...
            <ServerTable
                ...
                :columns="listAddCanvasRequestColumn"
                :key="key"
                :table-data="data"
                :loading="loading"
            ...
            >
             ...
             <FlexBox full jusEnd>
              <Button
                :trigger="submitCanvasRequest"
                class="tw-py-2 tw-px-14 tw-mr-4"
              >
                Submit
              </Button>
            </FlexBox>
                    ...
                    <p class="tw-text-sm tw-text-gray-600">
                      ...
                      {{ 
                        convertToTotalPieces({
                          qty_uom1: formRef.qty_uom1,
                          qty_uom2: formRef.qty_uom2,
                          qty_uom3: formRef.qty_uom3
                        }, formRef) 
                      }} pieces
                    </p>
                    </div>
