...
import {listEskalasiColumns} from "@/src/model/tableColumns/list-eskalasi/listEskalasi";
...
const searchQuery = async () => {
  ...
  try {
    ...
    await others.getAllStockOpnameFilter(clause);
    isServerTable.value = clause === '{"status_so =":"\'eskalasi\'"}';
  } catch (error) {
...
const buildClause = () => {
  searchEntries.value = [];
  searchEntries.value.push(["status_so =", "'eskalasi'"]);
...
const [data, count, loading, totalPage, key] = useFetchPaginate(
    `${paginateStockOpnameUrl}?id_cabang=${id_cabang}&status_so=eskalasi&`,
    {
      pagination,
      sorting,
      globalFilters,
...
<template>
  <FlexBox full flex-col>
    ...
    >
      <Card no-subheader>
        <template #header>List Eskalasi Stock Opname</template>
        <template #content>
          <ServerTable
              v-if="isServerTable"
              :columns="listEskalasiColumns"
              :key="key"
              :loading="loading"
              :table-data="data"
