...
const props = defineProps([
  ...
  "disabled",
  "noDynamicPlacing",
  "clearable", // Menambahkan prop untuk mengontrol apakah tombol clear ditampilkan
]);
...
// Fungsi baru untuk menghapus pilihan
const clearSelection = (event) => {
  // Menghentikan event agar tidak memicu openOptions
  ...
  <div
    ...
    <span
      ...
      :class="[
        ...
        selectedOptionText.length ? 'tw-text-gray-900' : 'tw-text-gray-500',
        disabled && 'tw-font-medium tw-text-gray-600/70',
      ]">
      <template v-if="selectedOptionText.length">
        {{
    ...
    <div class="tw-flex tw-items-center">
      <!-- Tombol X untuk menghapus pilihan -->
      <span
        v-if="selectedOptionText.length && props.clearable"
        @click="clearSelection($event)"
        class="mdi mdi-close tw-text-lg tw-mr-2 tw-text-gray-500 hover:tw-text-gray-700 tw-cursor-pointer"
        title="Hapus pilihan"></span>
