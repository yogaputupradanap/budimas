...
const beforeRoute = computed(() =>
  routeName.value.split("/").slice(-2)[0].split("-").join(" ")
);
const afterRoute = computed(() =>
  routeName.value.split("/").slice(-2)[1].split("-").join(" ")
);
...
<template>
  <div
    class="tw-w-full tw-flex tw-flex-col tw-gap-8 tw-mt-8 lg:tw-pl-4 lg:tw-pr-4 tw-pl-4 tw-pr-0"
  >
    <div
      v-if="!loginRoute"
