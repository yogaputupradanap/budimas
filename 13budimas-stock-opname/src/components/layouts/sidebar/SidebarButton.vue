...
const isActive = computed(() => {
  const routePath = route.path.toLowerCase().replace(/[^a-z0-9]/g, '');
  const buttonPath = props.to.toLowerCase().replace(/[^a-z0-9]/g, '');
  return routePath.includes(buttonPath.replace('/', ''));
});
</script>
...
<template>
  <div v-wave class="tw-w-full tw-overflow-hidden">
    <RouterLink
      :to="to"
      :class="[
        `tw-w-full tw-h-12 tw-flex tw-justify-start tw-px-3 tw-items-center  tw-rounded-md
    ...
    >
      <div class="tw-flex tw-justify-start tw-items-center tw-gap-2">
        ...
        <span id="navButtText" class="tw-font-bold tw-text-sm">{{ name }}</span>
      </div>
    </RouterLink>
  </div>
</template>
