...
const setFilterValue = debounce((e) => {
  ...
  const omitAction = columns.filter(
    (val) => ![`"actions"`, `"action"`, `"row_nums"`, `"row_num"`].includes(val)
  );

  const text = e.target.value;

  const filterValue = { text, columns: omitAction };
...
<template>
  <div
    ...
    <div
      v-if="!hideSearch"
      class="lg:tw-w-auto tw-w-full tw-flex tw-items-center">
      <span :class="['lg:tw-font-bold tw-font-normal tw-text-m tw-w-24']">
        search
      </span>
      <input
        v-if="server"
