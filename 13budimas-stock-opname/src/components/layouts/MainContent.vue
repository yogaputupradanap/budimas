...
const beforeRoute = computed(() => {
  const routeArr = routeName.value
    .split("/")
    .filter((val) => val !== "" && isNaN(parseInt(val)));
  if (routeArr.length === 1 && route.name == "Dashboard") {
    routeArr.unshift("home");
  }
  return routeArr.slice(-2)[0]?.split("-").join(" ");
});
const afterRoute = computed(() => {
  const routerArr = routeName.value
    .split("/")
    .filter((val) => val !== "" && isNaN(parseInt(val)));
  if (routerArr.length === 1 || (route.name && route.name !== "Dashboard")) {
    routerArr[0] = "list " + routerArr[0];
  }
  return routerArr
    .slice(-2)
    [routerArr.length == 1 ? 0 : 1]?.split("-")
    .join(" ");
});
    ...
    >
      ...
      >
        <BButton
          ...
          v-if="
            route.name &&
            route.name.trim() != 'Dashboard' &&
            route.name.trim() != 'Login'
          "
        ...
        <span class="text-black">{{ route.name }}</span>
      </SlideRightX>
      <SlideRightX
        :duration-enter="0.3"
        :duration-leave="0.3"
