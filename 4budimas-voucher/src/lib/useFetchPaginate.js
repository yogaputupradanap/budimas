...
export function useFetchPaginate(
  query = "",
  { pagination, sorting, globalFilters, initialSortColumn, asInitArgument = true } = {}
) {
  const data = ref([]);
  const count = ref(0);
  ...
  const getData = async () => {
    try {
      ...
      const initArgument = asInitArgument ? '?' : '&'
      const url = `${query}${initArgument}${mainParams.toString()}&${filterParam.toString()}`;
