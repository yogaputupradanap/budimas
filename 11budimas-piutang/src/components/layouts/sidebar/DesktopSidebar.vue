...
const userAccessButtons = computed(() => {
  // 1. Ambil data mentah
  const rawAccess = userStore.userAccess || [];

  // 2. Hilangkan duplikat berdasarkan 'name' agar tidak muncul 11-12 kali
  const uniqueNames = [...new Set(rawAccess.map(item => item.name))];

  // 3. Cocokkan dengan array 'buttons' lokal
  const filteredButtons = uniqueNames.map((name) => {
    return buttons.find((btn) => btn.route === name);
  }).filter(Boolean); // Hapus jika ada yang undefined

  // 4. Tambahkan Dashboard di awal (Hanya satu kali)
  return [
    {
      title: "Dashboard",
      route: "dashboard",
