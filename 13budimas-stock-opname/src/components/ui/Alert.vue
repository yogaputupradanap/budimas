...
<template>
  <Transition @enter="enter" @leave="leave" name="alert-transition">
    <BAlert
      ...
      class="tw-fixed tw-top-3 tw-right-[1%] md:tw-right-2 tw-z-50 tw-flex tw-flex-col tw-gap-2 md:tw-gap-4 tw-w-[98%] md:tw-w-auto">
      <span class="tw-text-sm">Message :</span>
      <div
        class="tw-w-full md:tw-w-80 tw-h-auto tw-max-h-32 tw-bg-white tw-rounded-lg tw-border tw-border-slate-300 tw-overflow-auto tw-bg-opacity-45">
        <p
          class="tw-text-xs tw-text-slate-700 tw-px-2 tw-py-3 first-letter:tw-uppercase">
          {{ message }}
        </p>
      </div>
      <BProgress
